...
public class GroundSoulFirePopulator extends Populator {
    ...
    public void populate(ChunkManager level, int chunkX, int chunkZ, NukkitRandom random, FullChunk chunk) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            for(int y : ys) {
                if(y <= 1) continue;
                this.level.setBlockAt(x, y, z, Block.SOUL_FIRE);
            }
        }
    }
    ...
    private ArrayList<Integer> getHighestWorkableBlocks(int x, int z) {
        ...
        for (y = 128; y > 0; y--) {
            ...
            if ((b == Block.SOUL_SOIL) && this.level.getBlockIdAt(x, y+1, z) == AIR) {
                blockYs.add(y+1);
            }
        }
        return blockYs;
